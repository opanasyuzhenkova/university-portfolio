#Лабораторная работа 6 Панасюженкова О.Д. 2-ИВТ-1
...
def load_params(file="/workspaces/university-portfolio/python-sem-3/lab-6-tables/params.ini"):
    """
    _________
...
def calculate(*args, **kwargs):
    ...
    elif args[len(args)-1] == 'D':
        n = len(args)-2
        x_sr = sum(args[0:len(args)-2]) / n
        res_sum = 0
        for x in args[0:len(args)-2]:
          res_sum += (x - x_sr)**2
        res = sqrt((1 / n) * res_sum)
        if type(res) is not output_type:
          res = output_type(res)
...
if __name__ == '__main__':
    def test_loading_params():
        ...
        assert PARAMS.get(
            'dest'
        ) == '/workspaces/university-portfolio/python-sem-3/lab-6-tables/history.txt', "Имя файла для записи истории вызовов функции calculate должно быть history.txt"
    ...
    res = calculate(6.123456789, 1.0000101,123, 1456.4,'*', **PARAMS)
    #write_log(*(6.123456789, 1.0000101), action='+', result=res)
    print_results(*(6.123456789, 1.0000101,123, 1456.4),'*',res)


    res = calculate(1,2,3,4,5,'D',**PARAMS)
    print_results(*(1,2,3,4,5), 'D',res)